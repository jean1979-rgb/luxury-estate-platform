...
export default function AdminTokkoPanel({
  ...
  return (
    <section className="flex h-[520px] min-h-[520px] flex-none flex-col overflow-hidden rounded-3xl border border-white/10 bg-white/[0.025]">
      <div className="border-b border-white/10 p-4">
        ...
        <input
          ...
          className="w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm text-white outline-none placeholder:text-white/35 focus:border-white/30"
        />
      </div>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
        {filtered.length === 0 ? (
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-sm text-white/50">
            No se encontraron propiedades en Tokko.
          </div>
        ) : (
          filtered.map((item) => {
