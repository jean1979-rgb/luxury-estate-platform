...
export default function PublicExperienceForm({ id }: { id?: string }) {
  ...
  async function save() {
    ...
    const res = await fetch(
      id ? `/api/admin/public/experiences/${id}` : "/api/admin/public/experiences",
      {
        method: id ? "PUT" : "POST",
  ...
  return (
    <div className="p-10 text-white max-w-3xl space-y-6">
      <h1 className="text-3xl font-light">{id ? "Edit Experience" : "New Experience"}</h1>
      ...
      <div className="flex gap-4">
        ...
        <button onClick={() => router.push("/admin/public/experiences")} className="border border-white/20 px-5 py-3 rounded-xl">
          Volver
        </button>
      </div>
    </div>
